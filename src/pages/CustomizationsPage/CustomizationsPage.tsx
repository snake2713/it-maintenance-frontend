...
import {T_CustomizationsFilters} from "modules/types.ts";
import CustomizationsTable from "components/CustomizationsTable/CustomizationsTable.tsx";

const CustomizationsPage = () => {

    const customizations = useAppSelector((state) => state.customizations.customizations)

    const {is_authenticated, is_superuser} = useAppSelector((state) => state.user)

    const filters = useAppSelector<T_CustomizationsFilters>((state) => state.customizations.filters)
    ...
    useEffect(() => {
        const intervalId = setInterval(handleFetchCustomizations, 2000)
        return () => clearInterval(intervalId)
    }, [filters]);

    const handleFetchCustomizations = () => dispatch(fetchCustomizations())

    const applyFilters = async (e) => {
        e.preventDefault()

        const filters:T_CustomizationsFilters = {
            status: status,
    ...
    return (
        <Container>
            ...
            {customizations.length ? <CustomizationsTable customizations={customizations} /> : <h3 className="text-center mt-5">Заказы не найдены</h3>}
        </Container>
    )
};
